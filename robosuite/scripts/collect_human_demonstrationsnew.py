...
def convert_to_joint_action_config(original_config):
    cfg = copy.deepcopy(original_config)

    for part_name, part_cfg in cfg["body_parts"].items():
        if part_name in ["left", "right"]:
            print(f"正在转换 {part_name} 为 JOINT_POSITION")

            # ✅ 先把 gripper 保存下来
            gripper_cfg = part_cfg.get("gripper", None)
...
if __name__ == "__main__":
    ...
    final_config = convert_to_joint_action_config(controller_config)
